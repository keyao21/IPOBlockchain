...
describe('Starting tests for ipoblock-frontEnd', function() {
  ...
  it('website title should be ipoblock-frontEnd', () => {
    page.navigateTo('/');
    return browser.getTitle().then((result)=>{
      expect(result).toBe('ipoblock-frontEnd');
    })
  });

  it('network-name should be ipoblock@0.2.3',() => {
    element(by.css('.network-name')).getWebElement()
    .then((webElement) => {
      return webElement.getText();
    })
    .then((txt) => {
      expect(txt).toBe('ipoblock@0.2.3.bna');
    });
  });

  it('navbar-brand should be ipoblock-frontEnd',() => {
    element(by.css('.navbar-brand')).getWebElement()
    .then((webElement) => {
      return webElement.getText();
    })
    .then((txt) => {
      expect(txt).toBe('ipoblock-frontEnd');
    });
  });
    ...
    it('engagementLetter table should have 11 columns',() => {
      page.navigateTo('/engagementLetter');
