...
describe('Starting tests for ipoblock', function() {
  ...
  it('website title should be ipoblock', () => {
    page.navigateTo('/');
    return browser.getTitle().then((result)=>{
      expect(result).toBe('ipoblock');
    })
  });

  it('network-name should be ipoblock@0.0.1',() => {
    element(by.css('.network-name')).getWebElement()
    .then((webElement) => {
      return webElement.getText();
    })
    .then((txt) => {
      expect(txt).toBe('ipoblock@0.0.1.bna');
    });
  });

  it('navbar-brand should be ipoblock',() => {
    element(by.css('.navbar-brand')).getWebElement()
    .then((webElement) => {
      return webElement.getText();
    })
    .then((txt) => {
      expect(txt).toBe('ipoblock');
    });
  });
